...
program P2
    ...
    ! altres variables
    !double precision, parameter :: kbol = 1.38064852d-26, navo = 6.02214076d23
    integer :: box_m
    ...
    do i  = 1, t_steps
        ! següent punt
        time = time + dt
        call velocity_verlet_andersen(nparts,box_l,cutoff,nu,sigma,dt,positions,velocities,pot,forces,1)
        if (opt1 == 1) then
        ...
        if (mod(i,k_steps) == 0) then
            ...
            write(15,*)
            do l = 1, nparts
                write(15,*) "A", positions(l,1), positions(l,2), positions(l,3)
            end do
        end if
    end do
    if (opt2 == 1) then
        ...
        !do i = 1, nparts
            !write(15,*) "A", positions(i,1), positions(i,2), positions(i,3)
        !end do
        close(15)
    end if
    ...
    print*, 'execution time (seconds) = ', abs(t_final-t_initial)
end program P2
