program Main
    ...
    nint = nparts*(nparts-1)/2

    call assign_particles(nparts)
    call assign_interactions(nparts)
    ...
    ! Melting
    if (melting == "Yes") then
        if (rank == 0) print*, "Melting the system at initial Temperature..."
    ...
    if (rank == 0) print*, "Computing the dynamics..."
    !------------------------------------------------------------- the system is ready --------------------------------------------!
    ! Initial state
    if (rank == 0) then
    ...
    if (rank==0) write(12,*) time*timeconv, kin*econv, pot*econv, tot*econv, tempi*tempconv, pres*pconv
    ...
    do iter  = 1, steps
        ...
        if (mod(iter,100) == 0) then
            if (rank == 0) write(13,*) time*timeconv, dr2*lconv**2/dble(nparts)/100.d0
            oldpos = newpos
        end if
        ...
        ! trajectories
        if (mod(iter,traj_steps) == 0 .and. rank==0) then
    ...
    if (rank == 0) print*, "processors =", nproc, "execution time =", final_time-initial_time
